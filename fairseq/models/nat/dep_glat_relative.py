...
class DEP_GLAT_RELATIVE(RelativeNonTransformerModel):
    ...
    def get_mask_output(self, mask_length=None, reference=None, samples=None, encoder_out=None, ):
        self.glat_warmup_steps = -1
        decoder_input, decoder_input_mask = self.decoder.get_copy_embedding(encoder_out, reference)

        embedding, tokens = self.get_mask_token(None, samples, decoder_input, update_nums=0, mask_length=mask_length,
    ...
    def get_mask_token(self, hidden_state, samples, decoder_input, update_nums, ratio=0.5, dep_model="random",
                       mask_length=None, **kwargs):
        ...
        if mask_length is None:
            ...
            mask_nums = ((references != self.pad).sum(-1) * ratio).int().cpu().tolist()
        else:
            mask_nums = mask_length
    ...
    def forward(
        ...
        prev_target_embedding, prev_output_tokens = self.get_mask_token(None, samples, decoder_input,
